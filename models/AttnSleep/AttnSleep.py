...
class MRCNN(nn.Module):
    def __init__(self, afr_reduced_cnn_size):
        ...
        self.features1 = nn.Sequential(
            nn.Conv1d(2, 64, kernel_size=50, stride=6, bias=False, padding=24),
            nn.BatchNorm1d(64),
            self.GELU,
        ...
        self.features2 = nn.Sequential(
            nn.Conv1d(2, 64, kernel_size=400, stride=50, bias=False, padding=200),
            nn.BatchNorm1d(64),
            self.GELU,
...
class AttnSleep(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        L = x_feat.size(-1)
        H = self.h
        L2 = ((L + H - 1) // H) * H  # next multiple of H
        if L2 != L:
            x_feat = F.pad(x_feat, (0, L2 - L))  # pad on the right
