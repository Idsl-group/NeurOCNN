...
from data_loaders.helpers import make_splits
...
class ECGResampledTestDataset(Dataset):
...
def create_ECG_dataloaders(
    ...
    num_workers=0,
    shuffle_train=True,
    train_fs=None,       # training sampling frequency
    train_len=None,      # in seconds
    test_fs_list=None,   # list of target Fs to test on
    test_len_list=None,  # in seconds
    keep_train_fs=False, # resample the test data back to train_fs (set True for traditional models)
    seed=42
):
    g = torch.Generator()
    g.manual_seed(seed)

    data = np.load(npz_folder)
    X_all = data["x"]
    y_all = data["y"]
    ch_names = None
    sfreq = data["fs"]
    ...
    train_idx, val_idx, test_idx = make_splits(
        y_all, train_size=train_size, val_size=val_size
    )

    if train_fs is None:
        train_ds = ECGDataset(X_all, y_all, train_idx)
        val_ds   = ECGDataset(X_all, y_all, val_idx)
        train_fs = base_fs
    else:
        train_ds = ECGResampledTestDataset(
                X_all, y_all, train_idx, base_fs=base_fs, train_fs=train_fs, target_fs=train_fs, target_len=train_len, keep_base_fs=False
            )
        val_ds   = ECGResampledTestDataset(
    ...
    for fs in test_fs_list:
        for length in test_len_list:
            test_ds = ECGResampledTestDataset(
...
def create_nfold_ecg_dataloaders(
    X_all, y_all,
